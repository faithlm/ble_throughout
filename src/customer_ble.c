...
static void scheduler_ble_command_process(void *p_event_data, uint16_t event_size)
{
    ...
    case BLE_COMMAND_TRANS_CONTROL:
        ...
        {
            NRF_LOG_ERROR("Invalid cmd params");
            rsp_data[1] = 1;
        }
...
static void nus_data_handler(ble_nus_evt_t *p_evt)
{
    ...
    {
        //只处理20字节以下的命令，当前命令帧不会超过20字节
        if (p_evt->params.rx_data.length < 20)
        {
            ret_code_t err_code = app_sched_event_put(p_evt->params.rx_data.p_data, p_evt->params.rx_data.length, scheduler_ble_command_process);
            APP_ERROR_CHECK(err_code);
        }
    }
}
